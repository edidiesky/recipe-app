...
export default function Veggie() {
  ...
  const [trendingVeggie, setTrendingVeggie] = useState([])
  /** getting the json data from the data base */
    const getTrendingVeggiedata = async () => {
      //set the localstorage

       const Checkrecipes = localStorage.getItem("trending")
      if (Checkrecipes) {
        setTrendingVeggie(JSON.parse(Checkrecipes))
      }else{
      ...
      console.log(data);
      setTrendingVeggie(data.recipes)
      }

    }
    useEffect(()=> {
      getTrendingVeggiedata()
    }, [])
  return (
    <Wrapper>

      <h3>Our trending Veggie Picks for you</h3>
      <Splide options={
        {
          perPage:4,
          gap:"5rem",
